...
/**
 * ZXing工具类
 * @see 	    用微信扫描GBK编码的中文二维码时出现乱码,用UTF-8编码时微信可正常识别
 ...
 * @see -----------------------------------------------------------------------------------------------------------------------
 * @version 目前仅支持二维码的生成和解析,生成二维码时支持添加logo头像
 */ 
public class ZXingUtil {
	private final static Logger logger = LoggerFactory.getLogger(ZXingUtil.class);
    private ZXingUtil(){} 
    ...
    public static String decodeQRCodeImage(String imagePath, String charset) { 
        ...
        if(null == image){ 
        	logger.info("Could not decode QRCodeImage"); 
            return ""; 
        } 
        ...
        try { 
			...
			logger.error("二维码图片[" + imagePath + "]解析失败,堆栈轨迹如下"); 
            return ""; 
        } 
    } 
    ...
    public static void main(String[] args) {
		...
		System.out.println(encodeQRCodeImage("http://www.baidu.com", "utf-8", "d:\\abc.png", 300, 300, "D:\\test.jpg"));
	}
}